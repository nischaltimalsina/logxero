...
const meetings = [
  {
    ...
    startDatetime: "2022-06-11T13:00",
...
export default function Calendar() {
  ...
  let selectedMonthMeetings = meetings.filter((meeting) =>
  ...
  return (
    <div className='w-full h-full flex flex-wrap gap-5 bg-white'>
      ...
      <div className='w-full sm:flex lg:block'>
        <section className='w-full sm:w-[50%] lg:w-full overflow-y-scroll bg-white p-4 '>
        ...
        <section className='w-full h-48 sm:w-[50%] lg:w-full overflow-y-scroll bg-white p-4 '>
          <h2 className='font-semibold text-gray-900'>
            All events for{" "}
            <time dateTime={format(firstDayCurrentMonth, "MMMM yyyy")}>
              {format(firstDayCurrentMonth, "MMMM yyyy")}
            </time>
          </h2>
          <ol className='mt-4 space-y-1 text-sm leading-6 text-gray-500'>
            {selectedMonthMeetings.length > 0 ? (
              selectedMonthMeetings.map((meeting) => (
                <MonthMeeting meeting={meeting} key={meeting.id} />
              ))
            ) : (
